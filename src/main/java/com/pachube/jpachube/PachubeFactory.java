...
public class PachubeFactory {
     ...
     * 
     * @param p
     *            Pachube object which is the gateway to the pachube service
     * @param s
     *            String which represents a feed object, this String should be
     *            well formed eeml.
    ...
    private static void getMisc(Feed f, Document doc) throws DOMException,
        ...
        if (c.getNodeType() == Node.ELEMENT_NODE) {
            ...
            if (fstNmElmnt != null) {
                fstNm = fstNmElmnt.getChildNodes();
                try {
                    f.setWebsite(new URL(((Node) fstNm.item(0)).getNodeValue()));
                } catch (Exception e3) {
    ...
    private static void getlocation(Feed f, Document doc)
        ...
        if (c != null) {
            n = c.getAttributes();
            l.setDomain(Domain.valueOf(n.getNamedItem("domain").getNodeValue()));
     ...
     * 
     * @param s
     *            String which represents a trigger object, this String should
     *            be well formed eeml.
            ...
            for (int i = 0; i < nodeLst.getLength(); i++) {
                ...
                if (c != null) {
                    ...
                    if (c.getNodeType() == Node.ELEMENT_NODE) {
                        ...
                        h = ((Node) fstNm.item(0));
                        if (h != null) {
                            t.setThreshold(Double.parseDouble(h.getNodeValue()));
                        }
